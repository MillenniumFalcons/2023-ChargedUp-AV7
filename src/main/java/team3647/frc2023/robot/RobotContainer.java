...
public class RobotContainer {
	...
	public final CubeWrist cubeWrist = new CubeWrist(
	...
	public final CubeShooterTop cubeShooterTop = new CubeShooterTop(
	...
	public final CubeShooterBottom cubeShooterBottom = new CubeShooterBottom(
	...
	final FlightDeck flightDeck = new FlightDeck(
	...
	private final AutoCommands autoCommands = new AutoCommands(swerve, SwerveDriveConstants.kDriveKinematics,
			superstructure);

	private final Command justScoreLevel3Cone = autoCommands.justScore(SuperstructureState.coneThreeReversed);
	private final Command justScoreLevel3Cube = autoCommands.justScore(SuperstructureState.cubeThreeReversed);

	private final Trigger globalEnableAutosteer = new Trigger(superstructure::autoSteerEnabled);
	private final BooleanSupplier goodForAutosteer = globalEnableAutosteer
	...
	private final Trigger wantCube = new Trigger(
			() -> superstructure.getWantedIntakePiece() == GamePiece.Cube || superstructure.isBottomF());

	private final Trigger almostThere = new Trigger(autoSteer::almostArrived);

	private final Trigger coControllerRightJoystickMoved = new Trigger(
			() -> Math.abs(coController.getRightStickY()) >= 0.2);
	private final Trigger coControllerLeftJoystickMoved = new Trigger(
			() -> Math.abs(coController.getLeftStickY()) >= 0.3);
}
