...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        scheduler.registerSubsystem(m_swerve, photonVisionCamera, m_printer);

        configureButtonBindings();
        configureDefaultCommands();
        configureSmartDashboardLogging();
        // m_swerve.setOdometry(
        //         PathPlannerTrajectories.spinStartPose, new
    ...
    private final PathPoint kOriginPoint =
            new PathPoint(
                    new Translation2d(Units.inchesToMeters(0), Units.inchesToMeters(0)),
                    new Rotation2d(Units.degreesToRadians(0)),
                    new Rotation2d(Units.degreesToRadians(180)));

    private final PathPoint chargeStationFront =
    ...
    private void configureButtonBindings() {
        ...
        mainController.buttonB.onTrue(
                new InstantCommand(() -> m_printer.addPose("target pose", () -> target)));
        mainController.buttonB.onTrue(
                new InstantCommand(
                        () -> {
                            new PrintCommand("Starting!")
                                    .andThen(
                                            m_swerve.getTrajectoryCommand(
                                                            m_swerve.getToPointATrajectory(
                                                                    getCalculatedTargetPose(
                                                                            new Translation2d(
                                                                                    1, 1))))
                                                    .withTimeout(8))
                                    .schedule();
        ...
        mainController
                .buttonX
                .whileTrue(
                        (new Balance(m_swerve, () -> false)
                                .until(
                                        () ->
                                                Math.abs(mainController.getLeftStickX()) > 0
                                                        || Math.abs(mainController.getLeftStickY())
                                                                > 0
                                                        || Math.abs(mainController.getRightStickX())
                                                                > 0)))
                .onFalse(
                        new SwerveDriveNoAim(m_swerve, () -> 0, () -> 0, () -> 0.1, () -> false)
                                .withTimeout(0.1));
    ...
    public void getTagPose() {
        if (photonVisionCamera.getHasTarget()) {
            for (PhotonTrackedTarget target : photonVisionCamera.getAllTargets()) {
                ...
                var fieldToTag =
                        robotPose3d
                                .transformBy(PhotonVisionConstants.robotToCam)
                                .transformBy(cameraToTagTransform);
                var fieldToTag2d =
                        new Pose2d(
                                new Translation2d(fieldToTag.getX(), fieldToTag.getY()),
                                new Rotation2d(fieldToTag.getRotation().getAngle()));
    ...
    private PathPoint getCalculatedTargetPose(Translation2d fromTag) {
        ...
        if (photonVisionCamera.getHasTarget()) {
            for (PhotonTrackedTarget target : photonVisionCamera.getAllTargets()) {
                ...
                if (target.getFiducialId() == 3 && target.getPoseAmbiguity() < 0.15) {
                    ...
                    var fromTag3d =
                            new Transform3d(
                                    new Translation3d(fromTag.getX(), fromTag.getY(), 0),
                                    new Rotation3d());
                    var fieldToTag =
                            robotPose3d
                                    .transformBy(PhotonVisionConstants.robotToCam)
        ...
        return new PathPoint(
                new Translation2d(avgTargetPose.getX(), avgTargetPose.getY()),
                new Rotation2d(),
    ...
    //     Translation2d fromTag = new Translation2d(Units.inchesToMeters(-23),
    // Units.inchesToMeters(48));
    //     var cameraToTagTransform = new Transform3d(limelightController.getCameraToTag(), new
    // Rotation3d());
    //     var robotPose3d = new Pose3d(m_swerve.getPose());
    //     var fromTag3d = new Transform3d(new Translation3d(fromTag.getX(), fromTag.getY(), 0), new
    // Rotation3d());
    //     var fieldToTag =
    // robotPose3d.transformBy(PhotonVisionConstants.robotToCam).transformBy(cameraToTagTransform);
    ...
    //     robotToFlushX = cameraToTagX - tagOffsetDepth + 0.295 *
    // Math.sin(Units.degreesToRadians(m_swerve.getRawHeading() - 180) + Math.atan(0.1 / 0.27));
    //     robotToFlushY = cameraToTagY + fromTag + 0.295 *
    // Math.cos(Units.degreesToRadians(m_swerve.getRawHeading() - 180) + Math.atan(0.1 / 0.27));
    //     //robotToFlushX = cameraToTagX - tagOffsetDepth +
    // Math.abs(Math.sin((Units.degreesToRadians(90 - m_swerve.getHeading()))) *
    // PhotonVisionConstants.robotToCam.getX());
    //     //robotToFlushY = cameraToTagY + tagOffsetSideway +
    // Math.abs(Math.cos(Units.degreesToRadians(m_swerve.getHeading())) *
    // PhotonVisionConstants.robotToCam.getY());
    ...
    //     return new PathPoint(new Translation2d(xSetPoint, ySetPoint), new Rotation2d(0), new
    // Rotation2d(Units.degreesToRadians(180)));
    //     // var cameraToTagTransform = new Transform3d(limelightController.getCameraToTag(), new
    // Rotation3d());
    //     // var robotPose3d = new Pose3d(m_swerve.getPose());
    //     // var fromTag3d = new Transform3d(new Translation3d(fromTag.getX(), fromTag.getY(), 0),
    // new Rotation3d());
    //     // var fieldToTag =
    // robotPose3d.transformBy(PhotonVisionConstants.robotToCam).transformBy(cameraToTagTransform);
    //     // var pose3d = fieldToTag.transformBy(fromTag3d);
    ...
    //     // return new PathPoint(new Translation2d(pose3d.getX(), pose3d.getY()), new
    // Rotation2d(), Rotation2d.fromDegrees(180));
    //  }

    private PathPoint getZeroPath() {
        Translation2d currentTranslation =
                new Translation2d(m_swerve.getPose().getX(), m_swerve.getPose().getY());
        return new PathPoint(
                currentTranslation, new Rotation2d(0), new Rotation2d(Units.degreesToRadians(180)));
    }
